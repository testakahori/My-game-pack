...
- **2026-07-12 更新**：リモートを `testakahori/My-game-pack`（**public**）に移行。
  public のため electron-updater の自動更新がトークン不要で利用可能になった。
  以下は旧記録：`testakahori/minecraft-tiktok-bridge`（private）。リポジトリの実体は `bridge_ui/ui` の
