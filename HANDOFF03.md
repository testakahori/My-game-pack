...
## 【2026-07-14 完了】v1.0.17 — 運営パスワード更新 & EXE 再ビルド

### 運営パスワードのローテーション
- 運営ログインパスワードを新しい値に変更（**新平文はリポジトリ・本ファイルに一切記載しない**。
  運営が別途保管）。
- `electron/main.cjs` の `OPERATOR_AUTH` を新しい salt/hash に差し替え済み
  （PBKDF2-SHA256・210,000回・32byte、方式は据え置き）。
- 検証済み: 新パスワードのみ通過 / 旧パスワード・誤入力は拒否 / `node --check` OK。
- 補足: `operatorLoginToken()` は `OPERATOR_AUTH.hash` を鍵に HMAC を計算するため、
  ハッシュ変更で既存の `app-config.json` 保存トークンは無効化され、次回起動で
  再ログインが必要になる（＝旧パスワードのトークン再利用を自動で締め出す挙動）。

### v1.0.17 ビルド
- version 1.0.16 → 1.0.17 に bump。`npm run pack:win` で Windows EXE 再ビルド。
- 同梱内容: 面白コマンド21件＋運営ログイン機能＋今回のパスワード更新。
- リリース手順（自動更新用）は従来通り: `release/` の Setup.exe・.blockmap・latest.yml の
  3点を GitHub Releases（testakahori/My-game-pack）へアップロード。

### 課金モデルの確定（2026-07-14 決定）
- 方針: **公式HP ＋ Stripe（決済） ＋ Cloudflare 無料枠（ライセンス認証API） ＋ Discord（コミュニティ）**。
- 個別ライセンス認証方式に移行する。現在のクライアント内蔵パスワード認証はテスター
  配布フェーズ用の暫定であり、有料開始時は Cloudflare Workers 側でライセンスキーを
  照合する方式へ切り替える（ソース公開のままでも鍵はサーバー管理なので迂回されにくい）。
...
- **ルーレット演出は2箇所に複製実装がある**：本番＝bridge/index.js の
  runRoulette / fireRouletteWinner、テスト発火（Bridge非経由）＝electron/main.cjs の
  fireDoumaEventMaybeRoulette。演出・抽選・count の仕様変更は必ず両方へ反映すること
  （count は「winner.repeat をそのまま使う」で統一済み）。
- **同梱コマンドtxtを廃止するときは main.cjs の BRIDGE_REMOVED_BUNDLE_FILES に追記**する
  こと。bridge差分同期はコピーのみで削除しないため、墓標リストに載せないと既存ユーザーの
  serverFolder/bridge/commands/minecraft/ に旧txtが残り続ける（giant/invisible/tiny で対応済み）。

## v1.0.20 自動更新404の修正（2026-07-16 深夜）

- 症状: 既存ユーザー（v1.0.18）の自動更新が
  `Cannot download ...MyGamePack-Bridge-UI-Setup-1.0.20.exe, status 404` で失敗。
- 原因: v1.0.20 はスペース入りファイル名のまま `gh release create` へアップロードしたため、
  GitHub がスペースを **ドット** に変換（`MyGamePack.Bridge.UI.Setup.1.0.20.exe`）。
  latest.yml はダッシュ区切り名を参照しており不一致に。v1.0.19 まで行っていた
  「アップロード前の手動リネーム」が漏れたのが直接原因。
- 対処①（公開済みリリースの復旧）: GitHub API で .exe / .blockmap をダッシュ区切り名へ
  リネーム済み。ダウンロードURLが 200 を返し、サイズが latest.yml と一致することを確認済み。
- 対処②（再発防止）: package.json の nsis.artifactName を
  `MyGamePack-Bridge-UI-Setup-${version}.${ext}` に変更。以後のビルドは最初から
  ダッシュ区切りで出力されるため、手動リネーム手順は不要になった。
  この修正を含めて v1.0.21 としてビルド・リリース（アプリの機能変更はなし。
  パッケージング修正＋ドキュメント更新のみ）。
- **今後のリリース手順**: `release/` の生成物 3 点（.exe / .blockmap / latest.yml）を
  そのままアップロードするだけでよい。アップロード後は必ず
  `releases/download/vX.Y.Z/MyGamePack-Bridge-UI-Setup-X.Y.Z.exe` が 200 を返すこと、
  アセット名が latest.yml の `path` と完全一致することを確認する。
