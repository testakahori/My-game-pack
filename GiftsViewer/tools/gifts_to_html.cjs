...
const inArg = pickArg("--in") || null;
const outDirArg = pickArg("--out") || null;

const inputPath = inArg
  ? path.resolve(inArg)
  : path.resolve(process.cwd(), "gifts.min.json");

const outDir = outDirArg
  ? path.resolve(outDirArg)
  : path.dirname(inputPath);

const outHtml = path.join(outDir, "gifts.html");

if (!fs.existsSync(inputPath)) {
  console.error("Input not found:", inputPath);
  console.error("Run: fetch_gifts.cjs first");
  process.exit(1);
}
