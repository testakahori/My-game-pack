...
contextBridge.exposeInMainWorld("mygamepack", {
  ...
  streamSessionStatus: () => ipcRenderer.invoke("stream:session:status"),
  streamSessionStart: (title) => ipcRenderer.invoke("stream:session:start", title),
  streamSessionEnd: (id, endedAt) => ipcRenderer.invoke("stream:session:end", { id, endedAt }),
  giftPanelSavePng: (dataUrl, filename) => ipcRenderer.invoke("image:saveGiftPanel", { dataUrl, filename }),
  ...
  worldSavesList: () => ipcRenderer.invoke('world:saves:list'),
  worldSavesSave: (name) => ipcRenderer.invoke('world:saves:save', name),
  worldSavesLoad: (id) => ipcRenderer.invoke('world:saves:load', id),
  operationsStatsExport: (streamId) => ipcRenderer.invoke('operations:stats:export', streamId),
