...
export default function WorldSavesPanel({ active }: { active: boolean }) {
  ...
  const run = async (saved?: MapSave) => {
    ...
    try {
      ...
      setNotice(saved ? '「' + saved.name + '」を読み込みました。読み込み前のMAPも一覧に残しています。' : '「' + result.saved!.name + '」にMAPを保存しました。');
      if (!saved) setName('');
  ...
  return <section className="map-saves-panel" aria-labelledby="map-saves-title">
    ...
    <div className="map-save-new"><label>セーブ名<input aria-label="MAPのセーブ名" maxLength={80} placeholder="例：ダイヤ集め・配信前" value={name} onChange={e => setName(e.target.value)} disabled={busy} /></label><button className="studio-primary" disabled={busy || loading || !world} onClick={() => void run()}>{busy ? '処理中…' : '現在のMAPをセーブ'}</button></div>
    <p className="panel-hint">名前が空欄なら日時で保存します。地形・持ち物・各ディメンションをまとめて保存。稼働中は自動で保存・停止してから操作し、完了後に再起動します。</p>
