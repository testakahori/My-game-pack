...
export default function MediaEffectsPage() {
  ...
  const test = () => void run(async () => {
    ...
    setTestResult('matched' in result && result.matched.length ? `発火: ${result.plays.map(play => `${play.name} ${play.count}回`).join(' / ')}。1回ずつ順番に再生します。` : '該当する有効な演出がありません。保存した条件を確認してください。');
  });
    ...
    <div className="media-columns">
      ...
      <section className="media-card"><div className="media-section-heading"><h2>2. 条件と見せ方</h2><button type="button" disabled={busy} onClick={() => edit(newRule(profile.id, assets))}>新しい演出</button></div>
        <div className="media-form">
          ...
          <label>きっかけ<select value={rule.trigger} onChange={e => patch({ trigger: e.target.value as MediaRule['trigger'] })}><option value="likes">いいね</option><option value="gift">指定ギフト</option><option value="coins">TikTokギフトのコイン数</option></select></label>
          {rule.trigger !== 'gift' ? <label>{rule.trigger === 'likes' ? '何いいねごと？' : '何コイン以上のギフト？'}<input type="number" min="1" max="10000000" value={rule.threshold} onChange={e => patch({ threshold: Number(e.target.value) })} /></label> : <label>ギフト<select value={rule.giftId} onChange={e => patch({ giftId: e.target.value })}><option value="">選んでください</option>{gifts.map(g => <option key={g.id} value={String(g.id)}>{g.name}（{g.diamond_count}コイン）</option>)}{rule.giftId && !gifts.some(g => String(g.id) === rule.giftId) && <option value={rule.giftId}>ID {rule.giftId}</option>}</select></label>}
          {rule.trigger === 'likes' && <p className="media-hint media-wide">{rule.threshold}いいねごとに1回。まとめず、1回ずつ順番に再生します。例：{rule.threshold * 3}いいねで3回。短時間の連打や再生待ちでも回数を減らしません。</p>}
          ...
          {rule.trigger === 'coins' && <label className="media-wide">コイン数の判定<select value={rule.coinMode} onChange={e => patch({ coinMode: e.target.value as 'unit' | 'streak' })}><option value="unit">ギフト1個のコイン数</option><option value="streak">連続ギフトの合計が到達したとき</option></select><small>TikTokギフトのコイン数です。例：薔薇1個＝1コイン。{rule.coinMode === 'unit' ? `${rule.threshold}コイン以上のギフト1個で発火します。` : `同じ連続ギフトの合計が${rule.threshold}コインに到達すると発火します。`}</small></label>}
          ...
          {rule.trigger !== 'likes' && <label>連続発火の間隔（秒）<input type="number" min="0" max="600" value={rule.cooldown} onChange={e => patch({ cooldown: Number(e.target.value) })} /></label>}
          </div></details>
